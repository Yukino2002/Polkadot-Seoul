...
#[ink::contract]
pub mod proxy {
    use ink_env::call::{ExecutionInput,Call,build_call};
    ...
    impl Proxy {
        ...
        #[ink(message)]
        pub fn erc20_balance_of(&self,owner:AccountId) -> Balance {
            ink_env::debug_println!("----- get caller {:?}",self.env().caller());
            ...
            .exec_input(
                ExecutionInput::new( ink_lang::selector_bytes!("balance_of").into()).
                push_arg(owner)
            )
            .returns::<Balance>()
            .fire().unwrap()
        ...
        #[ink(message)]
        pub fn erc20_transfer(&mut self,to:AccountId,value:Balance)->Result<(),u32> {
            ink_env::debug_println!("----- get transfer caller {:?}",self.env().caller());
           ...
           build_call::<DefaultEnvironment>()
            ...
            .exec_input(
                ExecutionInput::new( ink_lang::selector_bytes!("transfer").into())
                .push_arg(to)
                .push_arg(value)
            )
            .returns::<()>()
