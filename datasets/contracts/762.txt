...
mod erc721 {
    ...
    impl Erc721 {
        ...
        pub fn transfer(&mut self, destination: AccountId, token_id: TokenId) -> Result<(), Error> {
            ...
            self.transfer_token_from(&caller, &destination, token_id)?;
            Ok(())
        }
        ...
        pub fn burn(&mut self, token_id: TokenId) -> Result<(), Error> {
            ...
            let occupied = match token_owner.entry(token_id) {
        ...
        fn approve_for(&mut self, to: &AccountId, token_id: TokenId) -> Result<(), Error> {
            let caller = self.env().caller();
            ...
            if self.token_approvals.insert(token_id, *to).is_some() {
        ...
        fn exists(&self, token_id: TokenId) -> bool {
            self.token_owner.get(&token_id).is_some() && self.token_owner.contains_key(&token_id)
        }

        fn approved_or_owner(&self, from: Option<AccountId>, token_id: TokenId) -> bool {
            let owner = self.owner_of(token_id);
            from != Some(AccountId::from([0x0; 32]))
                && (from == owner
                    || from == self.token_approvals.get(&token_id).cloned()
        ...
        fn clear_approval(&mut self, token_id: TokenId) -> Result<(), Error> {
            if !self.token_approvals.contains_key(&token_id) {
                return Ok(())
            }

            match self.token_approvals.take(&token_id) {
                Some(_res) => Ok(()),
        ...
        fn add_token_to(&mut self, to: &AccountId, token_id: TokenId) -> Result<(), Error> {
            ...
            let vacant_token_owner = match token_owner.entry(token_id) {
